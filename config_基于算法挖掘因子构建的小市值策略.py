"""
标题：任务1】【D.C】基于算法挖掘因子构建的小市值策略_70.57%_-26.78%_2.64
链接：https://bbs.quantclass.cn/thread/64336
评价：少有的比择时前收益还高的策略
	亮点：最大回撤只有26，回撤次数还少、15年股灾还上涨
	缺点：发现3D周期挺好，但是1W就不行
	15年回撤大小：居然没有回撤
	22、23年涨跌：稳步上涨
	对数坐标轴斜率：log笔直、斜率大
...
# ====================================================================================================
strategy = {
    'name': '小市值策略20250808',  # 策略名
    'hold_period': '3D',  # 持仓周期，W 代表周，M 代表月
    'select_num': 5,  # 选股数量，可以是整数，也可以是小数，比如 0.1 表示选取 10% 的股票 7
    "factor_list": [  # 选股因子列表
        ('市值', True, None, 1),
        ('factor_20250801', True, 24, 1),
    ],
    "filter_list": [
        ("每股收益", None, 'val:>0', True),
        ("市值", None, 'pct:<0.2', True),
        ("收盘价", None, 'val:>2', True),
        ("月份", (1, 4), 'val:!=1', True),
        ("可用股票", None, 'val:==1', True),
        ("成交额缩量因子", (3, 24), 'val:<3', True),
        ("成交额缩量因子", (1, 24), 'val:<2', True),
    ]
}

days_listed = 250  # 上市至今交易天数

excluded_boards = ["cyb", "kcb", "bj"]

equity_timing = {'name': '三日涨跌幅择时', 'params': []}
