...
strategy = {
    ...
    "filter_list": [
        ...
        ('市值', None, 'pct:<=0.2'),
        # ('月份', [1], 'val:!=1'),  # 不在1月份选股
        ('Ret', 5, 'pct:<=0.5'), # 5日涨幅排名
        # ('换手率', 3, 'pct:>=0.5'), #换手>=5%
        ('月份', (1, 4), 'val:!=1') , # 不在1,4月份选股 对小市值策略非常有效的因子
...
#     "filter_list": [
#         # ('月份', [2], 'val:==1'),  # 只在2月份选股
#         ('流通市值', True, 'val:<3000000000'), #流通市值>30亿
#         ('成交额', False, 'rank:<200'), #取成交额排名最大的前5%
#         ('PE','单季', 'val:>0'),
#         # ('月份', [4,], 'val:!=1'),  # 不在4月份选股
#         # ('Ret', 5, 'pct:<=0.1'),
#         # ('收盘价', None, 'pct:<=0.5'),
...
# equity_timing = {"name": "移动平均线", "params": [10]}
# equity_timing = {"name": "动量_状态自适应择时", "params": [3, 5, 20, -0.05]} # 回撤缩小约40%，但收益大幅缩水
equity_timing = {"name": "止损择时", "params": [5, -0.05]}
