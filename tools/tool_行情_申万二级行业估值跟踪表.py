"""
使用方法：
https://bbs.quantclass.cn/thread/68712
"""
import pandas as pd
import numpy as np
import os
...
# 输出结果
output_path = r"/Users/wangminli/PycharmProjects/select-stock-group/data/行情_申万二级行业估值跟踪表.csv"
# 创建热力图
heatmap_path = r"/Users/wangminli/PycharmProjects/select-stock-group/data/行情_申万二级行业估值跟踪表_热力图.png"


# 设置中文字体，优先使用Microsoft YaHei，如果不可用则使用备选字体
plt.rcParams['font.family'] = ['PingFang HK', 'STHeiti', 'SimHei', 'Arial Unicode MS', 'sans-serif']
...
def calculate_stock_history(data_folder):
    ...
    for file_name in tqdm(csv_files):
        ...
        required_columns = [
            ...
            '净利润TTM', '净资产', '净利润(当季)', '新版申万二级行业名称'  # 修改为二级行业
        ]
...
def create_heatmap(data, output_path):
    ...
    heatmap_df = heatmap_data[['申万二级行业'] + available_columns].set_index('申万二级行业')  # 修改为二级行业
    ...
    fig, ax = plt.subplots(figsize=(10, 28))  # 增大图形尺寸以适应更多行业
    ...
    ax.set_xticklabels(heatmap_df.columns, rotation=0, ha='center', fontweight='bold')  # 增加标签旋转角度
    ax.set_yticklabels(ax.get_yticklabels(), fontweight='bold')  # 添加这行来加粗Y轴标签
    ax.set_ylabel('')

    # 调整布局并保存
    plt.tight_layout()
    plt.suptitle(f'申万二级行业估值跟踪表（数据截止{date_str}）', fontsize=16, y=0.98,fontweight='bold')  # 修改标题为二级行业
...
def analyze_industries_advanced(data_folder):
    ...
    # 计算每个行业在每个交易日的整体估值
    industry_daily = all_data.groupby(['新版申万二级行业名称', '交易日期']).agg({
        '总市值': 'sum',
        '净利润TTM': 'sum',
        '净资产': 'sum'
    }).reset_index()
    ...
    for industry in latest_industry_data['新版申万二级行业名称'].unique():
        industry_latest = latest_industry_data[
            latest_industry_data['新版申万二级行业名称'] == industry
            ].iloc[0]

        # 获取该行业过去10年的历史数据
        ten_years_ago = latest_date - pd.DateOffset(years=10)
        industry_history = industry_daily[
            (industry_daily['新版申万二级行业名称'] == industry) &
    ...
    # 计算综合百分位指标（市净率百分位 + 市盈率百分位）/ 2
    industry_df['combined_percentile'] = (industry_df['市盈率百分位'] + industry_df['市净率百分位']) / 2

    # 按综合百分位从大到小排序
    industry_df = industry_df.sort_values(by='combined_percentile', ascending=False)

    # 删除临时列，确保不在最终输出中显示
    industry_df = industry_df.drop(columns=['combined_percentile'])
    ...
    float_columns = industry_df.columns.difference(['申万二级行业', '包含股票数量', '交易日期'] + percent_columns)
