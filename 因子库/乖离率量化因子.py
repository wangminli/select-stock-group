...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    # 设置移动平均窗口，默认240分钟
    window = param if param is not None else 240

    # ======================== 计算因子 ===========================
    # 计算240分钟移动平均收盘价
    df['close_ma'] = df.groupby('股票代码')['收盘价'].transform(
        lambda x: x.rolling(window=window, min_periods=1).mean()
    )

    # 计算乖离率：(当前价-平均价)/平均价*100
    df[col_name] = ((df['收盘价'] - df['close_ma']) / df['close_ma']) * 100
    df[col_name] = df[col_name].fillna(0)

    # 清理临时列
    df.drop('close_ma', axis=1, inplace=True, errors='ignore')
