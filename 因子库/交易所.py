...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    # 赋值给因子df[col_name]=1
    df[col_name] = 1  # 沪A
    df.loc[df['股票代码'].str.startswith('sz0'), col_name] = 2  # 深A
    df.loc[df['股票代码'].str.startswith('sz3'), col_name] = 3  # 创业板
    df.loc[df['股票代码'].str.startswith('sh68'), col_name] = 4  # 科创板
    df.loc[df['股票代码'].str.startswith('bj'), col_name] = -5  # 北交所
