...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    # ======================== 计算因子 ===========================
    # 我们这里的市值因子使用总市值的数值
    today_turnover = df['成交额'] / df['流通市值']

    # 计算过去20日平均换手率（不包括当日）
    mean_turnover = today_turnover.rolling(window=param, min_periods=1).mean().shift(1)

    # 判断排除条件：
    # 1. 当日换手率 > 10%
    # 2. 当日换手率 > 2 * 20日平均换手率
    exclude_condition = (today_turnover > 0.1) & (today_turnover > 2 * mean_turnover)

    # 因子值：满足排除条件为1，否则为0
    df[col_name] = exclude_condition.astype(int)

    # 定义聚合规则：在转换周期时取最大值（只要周期内有一天满足就排除）
