...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    # ======================== 计算因子 ===========================
    # 计算boll
    df['middle'] = df['收盘价_复权'].rolling(20).mean()
    df['upper'] = df['middle'] + 2 * df['收盘价_复权'].rolling(20).std()
    df['lower'] = df['middle'] - 2 * df['收盘价_复权'].rolling(20).std()
    df[col_name] = 0
    
    # 使用布尔索引进行条件判断和赋值
    cond_upper = df['收盘价_复权'] > df['middle']  # 收盘价大于中轨
    cond_lower = df['收盘价_复权'] < df['middle']  # 收盘价小于中轨

    # 初始化因子列
    df[col_name] = 0

    # 根据条件赋值，收盘价在中轨上方则为正，在下方则为负，绝对值等于至中轨的偏离度/布林带的宽度
    df.loc[cond_upper, col_name] = (df['收盘价_复权'] - df['middle']) / (df['upper'] - df['middle'])  
    df.loc[cond_lower, col_name] = -1 * (df['收盘价_复权'] - df['middle']) / (df['lower'] - df['middle'])  
