...
large_scale_factor_28_fitness_0_542_20250812_175508: 基于遗传规划算法生成的因子28，适应度: 0.542236
适应度分数: 0.542236
原始GP表达式: CORRELATION20(TS_MAX(VWAP_PROXY, NUMBER_4), TS_MAX(BODY_SIZE, NUMBER_6))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_28_fitness_0_542_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan)).rolling(window=4, min_periods=1).max()).rolling(window=20).corr(((np.abs(o - c)).rolling(window=6, min_periods=1).max())))  # 原始GP表达式: CORRELATION20(TS_MAX(VWAP_PROXY, NUMBER_4), TS_MAX(BODY_SIZE, NUMBER_6))
