...
large_scale_factor_25_fitness_0_366_20250812_175508: 基于遗传规划算法生成的因子25，适应度: 0.365560
适应度分数: 0.365560
原始GP表达式: ABS_DIFF(TS_EMA(VOLUME_CHANGE_1D, NUMBER_11), CORRELATION60(LOW, LOW_CLOSE_DIFF))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_25_fitness_0_366_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.abs((v.pct_change(1)).ewm(span=11, adjust=False).mean() - (l).rolling(window=60).corr((l - c))))  # 原始GP表达式: ABS_DIFF(TS_EMA(VOLUME_CHANGE_1D, NUMBER_11), CORRELATION60(LOW, LOW_CLOSE_DIFF))
