...
large_scale_factor_18_fitness_0_410_20250812_175508: 基于遗传规划算法生成的因子18，适应度: 0.409635
适应度分数: 0.409635
原始GP表达式: DIVIDE(TS_EMA(HIGH_LOW_DIFF, NUMBER_31), TS_LAG(HLCC_WEIGHTED, NUMBER_48))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_18_fitness_0_410_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - l).ewm(span=31, adjust=False).mean() / ((h + l + 2*c) / 4).shift(48)))  # 原始GP表达式: DIVIDE(TS_EMA(HIGH_LOW_DIFF, NUMBER_31), TS_LAG(HLCC_WEIGHTED, NUMBER_48))
