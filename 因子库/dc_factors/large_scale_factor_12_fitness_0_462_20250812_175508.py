...
large_scale_factor_12_fitness_0_462_20250812_175508: 基于遗传规划算法生成的因子12，适应度: 0.461989
适应度分数: 0.461989
原始GP表达式: SUBTRACT(CORRELATION60(HIGH, LIUTONGSHI), SPEARMAN5(HIGH_LOW_DIFF, AVG_PRICE))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_12_fitness_0_462_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h).rolling(window=60).corr((mc_circ)) - (h - l).rolling(window=5).corr((amt / v.replace(0, np.nan)), method="spearman")))  # 原始GP表达式: SUBTRACT(CORRELATION60(HIGH, LIUTONGSHI), SPEARMAN5(HIGH_LOW_DIFF, AVG_PRICE))
