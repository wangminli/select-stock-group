...
large_scale_factor_62_fitness_0_530_20250812_175508: 基于遗传规划算法生成的因子62，适应度: 0.530454
适应度分数: 0.530454
原始GP表达式: BETA5(MINIMUM(VOLUME, VOLUME_PER_RANGE), SQUARED_DIFF(VOLUME, NUMBER_17))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_62_fitness_0_530_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((np.minimum(v, v / (h - l).replace(0, np.nan))).rolling(window=5).cov(((v - 17) ** 2)) / ((v - 17) ** 2).rolling(window=5).var())))  # 原始GP表达式: BETA5(MINIMUM(VOLUME, VOLUME_PER_RANGE), SQUARED_DIFF(VOLUME, NUMBER_17))
