...
large_scale_factor_17_fitness_0_423_20250812_175508: 基于遗传规划算法生成的因子17，适应度: 0.423254
适应度分数: 0.423254
原始GP表达式: SQUARED_DIFF(TS_STD(LOW_CLOSE_RATIO, NUMBER_42), BETA10(HLC_TYPICAL, DIRECTIONAL_VOLUME))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_17_fitness_0_423_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((l / c.replace(0, np.nan)).rolling(window=42, min_periods=1).std() - ((((h + l + c) / 3).rolling(window=10).cov((v * np.sign(c - o))) / (v * np.sign(c - o)).rolling(window=10).var()))) ** 2)  # 原始GP表达式: SQUARED_DIFF(TS_STD(LOW_CLOSE_RATIO, NUMBER_42), BETA10(HLC_TYPICAL, DIRECTIONAL_VOLUME))
