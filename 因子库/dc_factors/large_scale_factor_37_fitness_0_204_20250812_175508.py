...
large_scale_factor_37_fitness_0_204_20250812_175508: 基于遗传规划算法生成的因子37，适应度: 0.203842
适应度分数: 0.203842
原始GP表达式: TS_RANGE(TS_STD(VOLUME_WEIGHTED_PRICE, NUMBER_30), NUMBER_45)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_37_fitness_0_204_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((amt / v.replace(0, np.nan)).rolling(window=30, min_periods=1).std()).rolling(window=45, min_periods=1).max()) - (((amt / v.replace(0, np.nan)).rolling(window=30, min_periods=1).std()).rolling(window=45, min_periods=1).min())))  # 原始GP表达式: TS_RANGE(TS_STD(VOLUME_WEIGHTED_PRICE, NUMBER_30), NUMBER_45)
