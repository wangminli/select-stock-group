...
large_scale_factor_32_fitness_0_316_20250812_175508: 基于遗传规划算法生成的因子32，适应度: 0.315831
适应度分数: 0.315831
原始GP表达式: TS_SUM(CORRELATION10(AVG_PRICE, LOWER_SHADOW), NUMBER_52)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_32_fitness_0_316_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan)).rolling(window=10).corr((np.minimum(o, c) - l))).rolling(window=52, min_periods=1).sum())  # 原始GP表达式: TS_SUM(CORRELATION10(AVG_PRICE, LOWER_SHADOW), NUMBER_52)
