...
large_scale_factor_22_fitness_0_286_20250812_175508: 基于遗传规划算法生成的因子22，适应度: 0.285862
适应度分数: 0.285862
原始GP表达式: TS_MEDIAN(BETA20(RETURN10, LOW_CLOSE_RATIO), NUMBER_23)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_22_fitness_0_286_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((c.pct_change(10)).rolling(window=20).cov((l / c.replace(0, np.nan))) / (l / c.replace(0, np.nan)).rolling(window=20).var()))).rolling(window=23, min_periods=1).median())  # 原始GP表达式: TS_MEDIAN(BETA20(RETURN10, LOW_CLOSE_RATIO), NUMBER_23)
