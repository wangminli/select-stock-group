...
large_scale_factor_87_fitness_0_283_20250812_175508: 基于遗传规划算法生成的因子87，适应度: 0.283357
适应度分数: 0.283357
原始GP表达式: DIVIDE(TS_MAX(LIUTONGSHI, NUMBER_48), TS_MA(VOLUME, NUMBER_56))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_87_fitness_0_283_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((mc_circ).rolling(window=48, min_periods=1).max() / (v).rolling(window=56, min_periods=1).mean()))  # 原始GP表达式: DIVIDE(TS_MAX(LIUTONGSHI, NUMBER_48), TS_MA(VOLUME, NUMBER_56))
