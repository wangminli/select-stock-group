...
large_scale_factor_98_fitness_0_550_20250812_175508: 基于遗传规划算法生成的因子98，适应度: 0.550189
适应度分数: 0.550189
原始GP表达式: CORRELATION10(MAXIMUM(VOLUME_WEIGHTED_PRICE, ZONGSHIZHI), HIGH_OPEN_DIFF)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_98_fitness_0_550_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.maximum(amt / v.replace(0, np.nan), mc_total)).rolling(window=10).corr((h - o)))  # 原始GP表达式: CORRELATION10(MAXIMUM(VOLUME_WEIGHTED_PRICE, ZONGSHIZHI), HIGH_OPEN_DIFF)
