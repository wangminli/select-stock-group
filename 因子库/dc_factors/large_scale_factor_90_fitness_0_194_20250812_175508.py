...
large_scale_factor_90_fitness_0_194_20250812_175508: 基于遗传规划算法生成的因子90，适应度: 0.194076
适应度分数: 0.194076
原始GP表达式: CORRELATION60(TS_SUM(HIGH_CLOSE_DIFF, NUMBER_3), TS_RANGE(ZONGSHIZHI, NUMBER_38))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_90_fitness_0_194_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - c).rolling(window=3, min_periods=1).sum()).rolling(window=60).corr(((((mc_total).rolling(window=38, min_periods=1).max()) - ((mc_total).rolling(window=38, min_periods=1).min())))))  # 原始GP表达式: CORRELATION60(TS_SUM(HIGH_CLOSE_DIFF, NUMBER_3), TS_RANGE(ZONGSHIZHI, NUMBER_38))
