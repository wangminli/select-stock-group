...
large_scale_factor_95_fitness_0_292_20250812_175508: 基于遗传规划算法生成的因子95，适应度: 0.292438
适应度分数: 0.292438
原始GP表达式: TS_MAX(SPEARMAN20(TRUE_RANGE, CLOSE), NUMBER_6)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_95_fitness_0_292_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - l).rolling(window=20).corr((c), method="spearman")).rolling(window=6, min_periods=1).max())  # 原始GP表达式: TS_MAX(SPEARMAN20(TRUE_RANGE, CLOSE), NUMBER_6)
