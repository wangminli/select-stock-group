...
large_scale_factor_2_fitness_0_749_20250812_175508: 基于遗传规划算法生成的因子2，适应度: 0.749236
适应度分数: 0.749236
原始GP表达式: BETA20(MULTIPLY(MINIMUM(HIGH_LOW_DIFF, CLOSE), NUMBER_36), HIGH_OPEN_DIFF)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_2_fitness_0_749_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((np.minimum(h - l, c) * 36)).rolling(window=20).cov((h - o)) / (h - o).rolling(window=20).var())))  # 原始GP表达式: BETA20(MULTIPLY(MINIMUM(HIGH_LOW_DIFF, CLOSE), NUMBER_36), HIGH_OPEN_DIFF)
