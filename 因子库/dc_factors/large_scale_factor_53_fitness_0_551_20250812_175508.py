...
large_scale_factor_53_fitness_0_551_20250812_175508: 基于遗传规划算法生成的因子53，适应度: 0.551247
适应度分数: 0.551247
原始GP表达式: BETA10(SUBTRACT(TRUE_RANGE, NUMBER_31), DELTA(OC_MID, NUMBER_16))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_53_fitness_0_551_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((h - l - 31)).rolling(window=10).cov((((o + c) / 2).diff(16))) / (((o + c) / 2).diff(16)).rolling(window=10).var())))  # 原始GP表达式: BETA10(SUBTRACT(TRUE_RANGE, NUMBER_31), DELTA(OC_MID, NUMBER_16))
