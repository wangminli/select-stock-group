...
large_scale_factor_50_fitness_0_727_20250812_175508: 基于遗传规划算法生成的因子50，适应度: 0.727417
适应度分数: 0.727417
原始GP表达式: BETA10(MINIMUM(HIGH_LOW_DIFF, MAXIMUM(LOW_OPEN_DIFF, AVG_PRICE)), HLCC_WEIGHTED)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_50_fitness_0_727_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((np.minimum(h - l, np.maximum(l - o, amt / v.replace(0, np.nan)))).rolling(window=10).cov(((h + l + 2*c) / 4)) / ((h + l + 2*c) / 4).rolling(window=10).var())))  # 原始GP表达式: BETA10(MINIMUM(HIGH_LOW_DIFF, MAXIMUM(LOW_OPEN_DIFF, AVG_PRICE)), HLCC_WEIGHTED)
