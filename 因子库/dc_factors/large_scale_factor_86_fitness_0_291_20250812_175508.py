...
large_scale_factor_86_fitness_0_291_20250812_175508: 基于遗传规划算法生成的因子86，适应度: 0.290580
适应度分数: 0.290580
原始GP表达式: TS_MA(DIVIDE(TURNOVER_RATIO, FREE_FLOAT_RATIO), NUMBER_51)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_86_fitness_0_291_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v / mc_circ.replace(0, np.nan) / mc_circ / mc_total.replace(0, np.nan))).rolling(window=51, min_periods=1).mean())  # 原始GP表达式: TS_MA(DIVIDE(TURNOVER_RATIO, FREE_FLOAT_RATIO), NUMBER_51)
