...
large_scale_factor_13_fitness_0_402_20250812_175508: 基于遗传规划算法生成的因子13，适应度: 0.402122
适应度分数: 0.402122
原始GP表达式: MULTIPLY(DIVIDE(SQUARED_DIFF(LOWER_SHADOW, NUMBER_54), TS_MA(TURNOVER_VALUE, NUMBER_47)), NUMBER_53)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_13_fitness_0_402_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((np.minimum(o, c) - l - 54) ** 2 / (v * c).rolling(window=47, min_periods=1).mean()) * 53))  # 原始GP表达式: MULTIPLY(DIVIDE(SQUARED_DIFF(LOWER_SHADOW, NUMBER_54), TS_MA(TURNOVER_VALUE, NUMBER_47)), NUMBER_53)
