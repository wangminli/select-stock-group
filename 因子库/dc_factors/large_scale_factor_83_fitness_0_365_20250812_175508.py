...
large_scale_factor_83_fitness_0_365_20250812_175508: 基于遗传规划算法生成的因子83，适应度: 0.364576
适应度分数: 0.364576
原始GP表达式: ADD(BETA5(HIGH_OPEN_DIFF, OC_MID), NUMBER_44)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_83_fitness_0_365_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((h - o).rolling(window=5).cov(((o + c) / 2)) / ((o + c) / 2).rolling(window=5).var())) + 44))  # 原始GP表达式: ADD(BETA5(HIGH_OPEN_DIFF, OC_MID), NUMBER_44)
