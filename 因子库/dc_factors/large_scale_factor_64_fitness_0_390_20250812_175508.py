...
large_scale_factor_64_fitness_0_390_20250812_175508: 基于遗传规划算法生成的因子64，适应度: 0.389896
适应度分数: 0.389896
原始GP表达式: MAXIMUM(BETA5(LOW_OPEN_DIFF, CLOSE), TS_MIN(PRICE_CHANGE_3D, NUMBER_30))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_64_fitness_0_390_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.maximum((((l - o).rolling(window=5).cov((c)) / (c).rolling(window=5).var())), (c.pct_change(3)).rolling(window=30, min_periods=1).min()))  # 原始GP表达式: MAXIMUM(BETA5(LOW_OPEN_DIFF, CLOSE), TS_MIN(PRICE_CHANGE_3D, NUMBER_30))
