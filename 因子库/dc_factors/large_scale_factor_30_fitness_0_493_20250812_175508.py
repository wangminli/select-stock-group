...
large_scale_factor_30_fitness_0_493_20250812_175508: 基于遗传规划算法生成的因子30，适应度: 0.492765
适应度分数: 0.492765
原始GP表达式: TS_EMA(CORRELATION10(VWAP_PROXY, ZONGSHIZHI), NUMBER_22)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_30_fitness_0_493_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan)).rolling(window=10).corr((mc_total))).ewm(span=22, adjust=False).mean())  # 原始GP表达式: TS_EMA(CORRELATION10(VWAP_PROXY, ZONGSHIZHI), NUMBER_22)
