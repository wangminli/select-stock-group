...
large_scale_factor_26_fitness_0_555_20250812_175508: 基于遗传规划算法生成的因子26，适应度: 0.554654
适应度分数: 0.554654
原始GP表达式: BETA20(SQUARED_DIFF(NUMBER_24, LOW_CLOSE_DIFF), ADD(BODY_SIZE, HLCC_WEIGHTED))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_26_fitness_0_555_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((24 - l - c) ** 2).rolling(window=20).cov(((np.abs(o - c) + (h + l + 2*c) / 4))) / ((np.abs(o - c) + (h + l + 2*c) / 4)).rolling(window=20).var())))  # 原始GP表达式: BETA20(SQUARED_DIFF(NUMBER_24, LOW_CLOSE_DIFF), ADD(BODY_SIZE, HLCC_WEIGHTED))
