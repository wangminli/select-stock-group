...
large_scale_factor_34_fitness_0_243_20250812_175508: 基于遗传规划算法生成的因子34，适应度: 0.243281
适应度分数: 0.243281
原始GP表达式: MINIMUM(SUBTRACT(RETURN5, LOW_CLOSE_DEVIATION), LOW_CLOSE_RATIO)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_34_fitness_0_243_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.minimum((c.pct_change(5) - (c - l) / c.replace(0, np.nan)), l / c.replace(0, np.nan)))  # 原始GP表达式: MINIMUM(SUBTRACT(RETURN5, LOW_CLOSE_DEVIATION), LOW_CLOSE_RATIO)
