...
large_scale_factor_16_fitness_0_460_20250812_175508: 基于遗传规划算法生成的因子16，适应度: 0.460246
适应度分数: 0.460246
原始GP表达式: TS_MIN(CORRELATION60(VOLUME_PER_RANGE, LOW_OPEN_DIFF), NUMBER_25)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_16_fitness_0_460_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v / (h - l).replace(0, np.nan)).rolling(window=60).corr((l - o))).rolling(window=25, min_periods=1).min())  # 原始GP表达式: TS_MIN(CORRELATION60(VOLUME_PER_RANGE, LOW_OPEN_DIFF), NUMBER_25)
