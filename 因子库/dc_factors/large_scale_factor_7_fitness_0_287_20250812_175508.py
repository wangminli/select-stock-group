...
large_scale_factor_7_fitness_0_287_20250812_175508: 基于遗传规划算法生成的因子7，适应度: 0.287192
适应度分数: 0.287192
原始GP表达式: TS_STD(TS_VAR(HIGH_OPEN_RATIO, NUMBER_50), NUMBER_18)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_7_fitness_0_287_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h / o.replace(0, np.nan)).rolling(window=50, min_periods=1).var()).rolling(window=18, min_periods=1).std())  # 原始GP表达式: TS_STD(TS_VAR(HIGH_OPEN_RATIO, NUMBER_50), NUMBER_18)
