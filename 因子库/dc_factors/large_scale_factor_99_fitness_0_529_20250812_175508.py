...
large_scale_factor_99_fitness_0_529_20250812_175508: 基于遗传规划算法生成的因子99，适应度: 0.528810
适应度分数: 0.528810
原始GP表达式: CORRELATION5(DIVIDE(VOLUME_WEIGHTED_PRICE, VOLUME_PER_RANGE), OC_MID)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_99_fitness_0_529_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan) / v / (h - l).replace(0, np.nan))).rolling(window=5).corr(((o + c) / 2)))  # 原始GP表达式: CORRELATION5(DIVIDE(VOLUME_WEIGHTED_PRICE, VOLUME_PER_RANGE), OC_MID)
