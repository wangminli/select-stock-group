...
large_scale_factor_29_fitness_0_497_20250812_175508: 基于遗传规划算法生成的因子29，适应度: 0.497450
适应度分数: 0.497450
原始GP表达式: TS_STD(BETA20(AMOUNT, ZONGSHIZHI), NUMBER_20)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_29_fitness_0_497_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((amt).rolling(window=20).cov((mc_total)) / (mc_total).rolling(window=20).var()))).rolling(window=20, min_periods=1).std())  # 原始GP表达式: TS_STD(BETA20(AMOUNT, ZONGSHIZHI), NUMBER_20)
