...
large_scale_factor_66_fitness_0_645_20250812_175508: 基于遗传规划算法生成的因子66，适应度: 0.644940
适应度分数: 0.644940
原始GP表达式: TS_EMA(CORRELATION5(LIUTONGSHI, BODY_SIZE), NUMBER_2)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_66_fitness_0_645_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((mc_circ).rolling(window=5).corr((np.abs(o - c)))).ewm(span=2, adjust=False).mean())  # 原始GP表达式: TS_EMA(CORRELATION5(LIUTONGSHI, BODY_SIZE), NUMBER_2)
