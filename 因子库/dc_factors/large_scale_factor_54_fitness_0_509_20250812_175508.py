...
large_scale_factor_54_fitness_0_509_20250812_175508: 基于遗传规划算法生成的因子54，适应度: 0.509330
适应度分数: 0.509330
原始GP表达式: TS_MAX(CORRELATION20(SQUARED_DIFF(VOLUME_WEIGHTED_PRICE, NUMBER_40), SUBTRACT(DIRECTIONAL_VOLUME, NUMBER_37)), NUMBER_20)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_54_fitness_0_509_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((amt / v.replace(0, np.nan) - 40) ** 2).rolling(window=20).corr(((v * np.sign(c - o) - 37)))).rolling(window=20, min_periods=1).max())  # 原始GP表达式: TS_MAX(CORRELATION20(SQUARED_DIFF(VOLUME_WEIGHTED_PRICE, NUMBER_40), SUBTRACT(DIRECTIONAL_VOLUME, NUMBER_37)), NUMBER_20)
