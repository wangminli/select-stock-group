...
large_scale_factor_84_fitness_0_349_20250812_175508: 基于遗传规划算法生成的因子84，适应度: 0.348891
适应度分数: 0.348891
原始GP表达式: CORRELATION60(MULTIPLY(NUMBER_15, HIGH_OPEN_DIFF), ABS_DIFF(HIGH, HIGH_CLOSE_DIFF))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_84_fitness_0_349_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((15 * h - o)).rolling(window=60).corr((np.abs(h - h - c))))  # 原始GP表达式: CORRELATION60(MULTIPLY(NUMBER_15, HIGH_OPEN_DIFF), ABS_DIFF(HIGH, HIGH_CLOSE_DIFF))
