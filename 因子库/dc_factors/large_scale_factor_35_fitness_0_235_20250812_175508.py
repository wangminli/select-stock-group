...
large_scale_factor_35_fitness_0_235_20250812_175508: 基于遗传规划算法生成的因子35，适应度: 0.235349
适应度分数: 0.235349
原始GP表达式: TS_STD(ABS_DIFF(VWAP_PROXY, NUMBER_48), NUMBER_8)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_35_fitness_0_235_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.abs(amt / v.replace(0, np.nan) - 48)).rolling(window=8, min_periods=1).std())  # 原始GP表达式: TS_STD(ABS_DIFF(VWAP_PROXY, NUMBER_48), NUMBER_8)
