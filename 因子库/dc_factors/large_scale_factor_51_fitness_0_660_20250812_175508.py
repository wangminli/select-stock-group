...
large_scale_factor_51_fitness_0_660_20250812_175508: 基于遗传规划算法生成的因子51，适应度: 0.660222
适应度分数: 0.660222
原始GP表达式: BETA10(DIVIDE(MAXIMUM(BODY_SIZE, LOW_CLOSE_DIFF), NUMBER_36), CLOSE)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_51_fitness_0_660_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((np.maximum(np.abs(o - c), l - c) / 36)).rolling(window=10).cov((c)) / (c).rolling(window=10).var())))  # 原始GP表达式: BETA10(DIVIDE(MAXIMUM(BODY_SIZE, LOW_CLOSE_DIFF), NUMBER_36), CLOSE)
