...
large_scale_factor_15_fitness_0_523_20250812_175508: 基于遗传规划算法生成的因子15，适应度: 0.522788
适应度分数: 0.522788
原始GP表达式: CORRELATION5(TS_MEDIAN(HL_MID, NUMBER_36), TS_EMA(HIGH, NUMBER_47))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_15_fitness_0_523_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((h + l) / 2).rolling(window=36, min_periods=1).median()).rolling(window=5).corr(((h).ewm(span=47, adjust=False).mean())))  # 原始GP表达式: CORRELATION5(TS_MEDIAN(HL_MID, NUMBER_36), TS_EMA(HIGH, NUMBER_47))
