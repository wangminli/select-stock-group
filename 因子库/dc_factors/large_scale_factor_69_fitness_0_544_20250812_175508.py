...
large_scale_factor_69_fitness_0_544_20250812_175508: 基于遗传规划算法生成的因子69，适应度: 0.543534
适应度分数: 0.543534
原始GP表达式: TS_MEDIAN(CORRELATION5(HIGH_OPEN_DIFF, ZONGSHIZHI), NUMBER_39)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_69_fitness_0_544_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - o).rolling(window=5).corr((mc_total))).rolling(window=39, min_periods=1).median())  # 原始GP表达式: TS_MEDIAN(CORRELATION5(HIGH_OPEN_DIFF, ZONGSHIZHI), NUMBER_39)
