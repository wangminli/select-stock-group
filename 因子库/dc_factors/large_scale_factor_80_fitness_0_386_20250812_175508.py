...
large_scale_factor_80_fitness_0_386_20250812_175508: 基于遗传规划算法生成的因子80，适应度: 0.385862
适应度分数: 0.385862
原始GP表达式: TS_EMA(CORRELATION20(TRUE_RANGE, AMOUNT), NUMBER_5)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_80_fitness_0_386_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - l).rolling(window=20).corr((amt))).ewm(span=5, adjust=False).mean())  # 原始GP表达式: TS_EMA(CORRELATION20(TRUE_RANGE, AMOUNT), NUMBER_5)
