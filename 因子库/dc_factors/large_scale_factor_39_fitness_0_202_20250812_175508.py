...
large_scale_factor_39_fitness_0_202_20250812_175508: 基于遗传规划算法生成的因子39，适应度: 0.201538
适应度分数: 0.201538
原始GP表达式: TS_MIN(TS_MA(LIUTONGSHI, NUMBER_6), NUMBER_16)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_39_fitness_0_202_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((mc_circ).rolling(window=6, min_periods=1).mean()).rolling(window=16, min_periods=1).min())  # 原始GP表达式: TS_MIN(TS_MA(LIUTONGSHI, NUMBER_6), NUMBER_16)
