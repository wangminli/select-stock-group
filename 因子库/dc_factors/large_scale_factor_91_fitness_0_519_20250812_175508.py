...
large_scale_factor_91_fitness_0_519_20250812_175508: 基于遗传规划算法生成的因子91，适应度: 0.519205
适应度分数: 0.519205
原始GP表达式: CORRELATION5(MULTIPLY(VOLUME_WEIGHTED_PRICE, LOW_CLOSE_RATIO), SUBTRACT(NUMBER_38, ZONGSHIZHI))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_91_fitness_0_519_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan) * l / c.replace(0, np.nan))).rolling(window=5).corr(((38 - mc_total))))  # 原始GP表达式: CORRELATION5(MULTIPLY(VOLUME_WEIGHTED_PRICE, LOW_CLOSE_RATIO), SUBTRACT(NUMBER_38, ZONGSHIZHI))
