...
large_scale_factor_97_fitness_0_242_20250812_175508: 基于遗传规划算法生成的因子97，适应度: 0.242195
适应度分数: 0.242195
原始GP表达式: ADD(TS_RANK(VWAP_PROXY, NUMBER_14), TURNOVER)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_97_fitness_0_242_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan)).rolling(window=14).apply(lambda s: s.rank(pct=True).iloc[-1]) + v / mc_circ.replace(0, np.nan)))  # 原始GP表达式: ADD(TS_RANK(VWAP_PROXY, NUMBER_14), TURNOVER)
