...
large_scale_factor_20_fitness_0_342_20250812_175508: 基于遗传规划算法生成的因子20，适应度: 0.341855
适应度分数: 0.341855
原始GP表达式: CORRELATION10(TS_MAX(HIGH_OPEN_DIFF, NUMBER_5), TS_MIN(HLCC_WEIGHTED, NUMBER_37))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_20_fitness_0_342_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - o).rolling(window=5, min_periods=1).max()).rolling(window=10).corr((((h + l + 2*c) / 4).rolling(window=37, min_periods=1).min())))  # 原始GP表达式: CORRELATION10(TS_MAX(HIGH_OPEN_DIFF, NUMBER_5), TS_MIN(HLCC_WEIGHTED, NUMBER_37))
