...
large_scale_factor_1_fitness_0_839_20250812_175508: 基于遗传规划算法生成的因子1，适应度: 0.838867
适应度分数: 0.838867
原始GP表达式: ADD(CORRELATION20(TRUE_RANGE, VOLUME_PER_RANGE), BETA5(VOLUME_WEIGHTED_PRICE, ZONGSHIZHI))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_1_fitness_0_839_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h - l).rolling(window=20).corr((v / (h - l).replace(0, np.nan))) + (((amt / v.replace(0, np.nan)).rolling(window=5).cov((mc_total)) / (mc_total).rolling(window=5).var()))))  # 原始GP表达式: ADD(CORRELATION20(TRUE_RANGE, VOLUME_PER_RANGE), BETA5(VOLUME_WEIGHTED_PRICE, ZONGSHIZHI))
