...
large_scale_factor_58_fitness_0_313_20250812_175508: 基于遗传规划算法生成的因子58，适应度: 0.312739
适应度分数: 0.312739
原始GP表达式: CORRELATION20(SUBTRACT(ABS_DIFF(SUBTRACT(NUMBER_13, LOWER_SHADOW), NUMBER_8), OPEN), SUBTRACT(AMOUNT, NUMBER_21))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_58_fitness_0_313_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((np.abs((13 - np.minimum(o, c) - l) - 8) - o)).rolling(window=20).corr(((amt - 21))))  # 原始GP表达式: CORRELATION20(SUBTRACT(ABS_DIFF(SUBTRACT(NUMBER_13, LOWER_SHADOW), NUMBER_8), OPEN), SUBTRACT(AMOUNT, NUMBER_21))
