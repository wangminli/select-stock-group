...
large_scale_factor_63_fitness_0_468_20250812_175508: 基于遗传规划算法生成的因子63，适应度: 0.468154
适应度分数: 0.468154
原始GP表达式: ADD(DIVIDE(LIUTONGSHI, VWAP_PROXY), MAXIMUM(HIGH_OPEN_RATIO, RETURN5))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_63_fitness_0_468_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((mc_circ / amt / v.replace(0, np.nan)) + np.maximum(h / o.replace(0, np.nan), c.pct_change(5))))  # 原始GP表达式: ADD(DIVIDE(LIUTONGSHI, VWAP_PROXY), MAXIMUM(HIGH_OPEN_RATIO, RETURN5))
