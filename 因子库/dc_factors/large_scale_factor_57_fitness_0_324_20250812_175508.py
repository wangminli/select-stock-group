...
large_scale_factor_57_fitness_0_324_20250812_175508: 基于遗传规划算法生成的因子57，适应度: 0.323989
适应度分数: 0.323989
原始GP表达式: SUBTRACT(SPEARMAN5(VOLUME, HLCC_WEIGHTED), TS_EMA(OPEN_POSITION, NUMBER_21))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_57_fitness_0_324_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v).rolling(window=5).corr(((h + l + 2*c) / 4), method="spearman") - ((o - l) / (h - l).replace(0, np.nan)).ewm(span=21, adjust=False).mean()))  # 原始GP表达式: SUBTRACT(SPEARMAN5(VOLUME, HLCC_WEIGHTED), TS_EMA(OPEN_POSITION, NUMBER_21))
