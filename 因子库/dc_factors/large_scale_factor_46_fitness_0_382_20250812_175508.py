...
large_scale_factor_46_fitness_0_382_20250812_175508: 基于遗传规划算法生成的因子46，适应度: 0.381897
适应度分数: 0.381897
原始GP表达式: TS_LAG(MAXIMUM(TURNOVER_VALUE, RANGE_VOLATILITY), NUMBER_25)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_46_fitness_0_382_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.maximum(v * c, (h - l) / (h + l).replace(0, np.nan))).shift(25))  # 原始GP表达式: TS_LAG(MAXIMUM(TURNOVER_VALUE, RANGE_VOLATILITY), NUMBER_25)
