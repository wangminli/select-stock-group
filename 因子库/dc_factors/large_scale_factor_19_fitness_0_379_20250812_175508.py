...
large_scale_factor_19_fitness_0_379_20250812_175508: 基于遗传规划算法生成的因子19，适应度: 0.378817
适应度分数: 0.378817
原始GP表达式: MINIMUM(TS_RANK(HL_MID, NUMBER_15), TS_MA(TURNOVER_RATIO, NUMBER_55))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_19_fitness_0_379_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.minimum(((h + l) / 2).rolling(window=15).apply(lambda s: s.rank(pct=True).iloc[-1]), (v / mc_circ.replace(0, np.nan)).rolling(window=55, min_periods=1).mean()))  # 原始GP表达式: MINIMUM(TS_RANK(HL_MID, NUMBER_15), TS_MA(TURNOVER_RATIO, NUMBER_55))
