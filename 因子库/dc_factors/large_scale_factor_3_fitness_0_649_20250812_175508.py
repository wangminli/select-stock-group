...
large_scale_factor_3_fitness_0_649_20250812_175508: 基于遗传规划算法生成的因子3，适应度: 0.648874
适应度分数: 0.648874
原始GP表达式: BETA10(DIVIDE(AMOUNT, NUMBER_55), DELTA(LIUTONGSHI, NUMBER_11))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_3_fitness_0_649_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((amt / 55)).rolling(window=10).cov(((mc_circ).diff(11))) / ((mc_circ).diff(11)).rolling(window=10).var())))  # 原始GP表达式: BETA10(DIVIDE(AMOUNT, NUMBER_55), DELTA(LIUTONGSHI, NUMBER_11))
