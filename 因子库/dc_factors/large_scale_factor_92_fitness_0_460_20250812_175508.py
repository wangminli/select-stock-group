...
large_scale_factor_92_fitness_0_460_20250812_175508: 基于遗传规划算法生成的因子92，适应度: 0.460288
适应度分数: 0.460288
原始GP表达式: BETA20(MAXIMUM(TRUE_RANGE, HLC_TYPICAL), ADD(LOW, HLC_TYPICAL))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_92_fitness_0_460_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((np.maximum(h - l, (h + l + c) / 3)).rolling(window=20).cov(((l + (h + l + c) / 3))) / ((l + (h + l + c) / 3)).rolling(window=20).var())))  # 原始GP表达式: BETA20(MAXIMUM(TRUE_RANGE, HLC_TYPICAL), ADD(LOW, HLC_TYPICAL))
