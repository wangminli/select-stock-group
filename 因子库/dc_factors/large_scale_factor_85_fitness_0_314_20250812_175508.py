...
large_scale_factor_85_fitness_0_314_20250812_175508: 基于遗传规划算法生成的因子85，适应度: 0.313867
适应度分数: 0.313867
原始GP表达式: ADD(SPEARMAN10(HIGH, VOLUME_WEIGHTED_PRICE), NUMBER_36)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_85_fitness_0_314_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((h).rolling(window=10).corr((amt / v.replace(0, np.nan)), method="spearman") + 36))  # 原始GP表达式: ADD(SPEARMAN10(HIGH, VOLUME_WEIGHTED_PRICE), NUMBER_36)
