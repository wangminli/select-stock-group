...
large_scale_factor_6_fitness_0_380_20250812_175508: 基于遗传规划算法生成的因子6，适应度: 0.380269
适应度分数: 0.380269
原始GP表达式: TS_SUM(TS_EMA(TURNOVER, NUMBER_12), NUMBER_37)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_6_fitness_0_380_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v / mc_circ.replace(0, np.nan)).ewm(span=12, adjust=False).mean()).rolling(window=37, min_periods=1).sum())  # 原始GP表达式: TS_SUM(TS_EMA(TURNOVER, NUMBER_12), NUMBER_37)
