...
large_scale_factor_68_fitness_0_555_20250812_175508: 基于遗传规划算法生成的因子68，适应度: 0.555027
适应度分数: 0.555027
原始GP表达式: MINIMUM(CORRELATION10(AMOUNT, VWAP_PROXY), TS_RANGE(AMOUNT_TO_MCAP, NUMBER_21))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_68_fitness_0_555_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.minimum((amt).rolling(window=10).corr((amt / v.replace(0, np.nan))), (((amt / mc_total.replace(0, np.nan)).rolling(window=21, min_periods=1).max()) - ((amt / mc_total.replace(0, np.nan)).rolling(window=21, min_periods=1).min()))))  # 原始GP表达式: MINIMUM(CORRELATION10(AMOUNT, VWAP_PROXY), TS_RANGE(AMOUNT_TO_MCAP, NUMBER_21))
