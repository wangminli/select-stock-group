...
large_scale_factor_94_fitness_0_449_20250812_175508: 基于遗传规划算法生成的因子94，适应度: 0.448714
适应度分数: 0.448714
原始GP表达式: CORRELATION60(MAXIMUM(VOLUME, VOLUME_PER_RANGE), SUBTRACT(VOLUME, DIRECTIONAL_VOLUME))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_94_fitness_0_449_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.maximum(v, v / (h - l).replace(0, np.nan))).rolling(window=60).corr(((v - v * np.sign(c - o)))))  # 原始GP表达式: CORRELATION60(MAXIMUM(VOLUME, VOLUME_PER_RANGE), SUBTRACT(VOLUME, DIRECTIONAL_VOLUME))
