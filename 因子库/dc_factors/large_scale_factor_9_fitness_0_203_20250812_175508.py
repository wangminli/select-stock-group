...
large_scale_factor_9_fitness_0_203_20250812_175508: 基于遗传规划算法生成的因子9，适应度: 0.203323
适应度分数: 0.203323
原始GP表达式: ABS_DIFF(TS_LAG(INTRADAY_VOLATILITY, NUMBER_56), NUMBER_19)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_9_fitness_0_203_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.abs((np.abs(c - o) / o.replace(0, np.nan)).shift(56) - 19))  # 原始GP表达式: ABS_DIFF(TS_LAG(INTRADAY_VOLATILITY, NUMBER_56), NUMBER_19)
