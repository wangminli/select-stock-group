...
large_scale_factor_100_20250812_175508: 基于遗传规划算法生成的因子100，适应度: 0.000000
适应度分数: 0.000000
原始GP表达式: CORRELATION20(SUBTRACT(MAXIMUM(HLC_TYPICAL, HLCC_WEIGHTED), ABS_DIFF(AVG_PRICE, BODY_SIZE)), LOW)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_100_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((np.maximum((h + l + c) / 3, (h + l + 2*c) / 4) - np.abs(amt / v.replace(0, np.nan) - np.abs(o - c)))).rolling(window=20).corr((l)))  # 原始GP表达式: CORRELATION20(SUBTRACT(MAXIMUM(HLC_TYPICAL, HLCC_WEIGHTED), ABS_DIFF(AVG_PRICE, BODY_SIZE)), LOW)
