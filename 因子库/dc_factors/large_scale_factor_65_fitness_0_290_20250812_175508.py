...
large_scale_factor_65_fitness_0_290_20250812_175508: 基于遗传规划算法生成的因子65，适应度: 0.289975
适应度分数: 0.289975
原始GP表达式: CORRELATION20(DELTA(MINIMUM(UPPER_SHADOW, BODY_SIZE), NUMBER_42), TS_MEDIAN(VOLUME, NUMBER_3))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_65_fitness_0_290_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((np.minimum(h - np.maximum(o, c), np.abs(o - c))).diff(42)).rolling(window=20).corr(((v).rolling(window=3, min_periods=1).median())))  # 原始GP表达式: CORRELATION20(DELTA(MINIMUM(UPPER_SHADOW, BODY_SIZE), NUMBER_42), TS_MEDIAN(VOLUME, NUMBER_3))
