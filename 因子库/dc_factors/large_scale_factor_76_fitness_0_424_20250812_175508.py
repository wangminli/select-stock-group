...
large_scale_factor_76_fitness_0_424_20250812_175508: 基于遗传规划算法生成的因子76，适应度: 0.423691
适应度分数: 0.423691
原始GP表达式: CORRELATION10(TS_RANGE(VOLUME, NUMBER_15), ABS_DIFF(VOLUME_PER_RANGE, NUMBER_57))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_76_fitness_0_424_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((v).rolling(window=15, min_periods=1).max()) - ((v).rolling(window=15, min_periods=1).min()))).rolling(window=10).corr((np.abs(v / (h - l).replace(0, np.nan) - 57))))  # 原始GP表达式: CORRELATION10(TS_RANGE(VOLUME, NUMBER_15), ABS_DIFF(VOLUME_PER_RANGE, NUMBER_57))
