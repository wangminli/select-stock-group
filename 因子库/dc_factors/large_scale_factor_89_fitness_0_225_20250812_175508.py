...
large_scale_factor_89_fitness_0_225_20250812_175508: 基于遗传规划算法生成的因子89，适应度: 0.225092
适应度分数: 0.225092
原始GP表达式: TS_RANGE(SQUARED_DIFF(NUMBER_36, PRICE_CHANGE_2D), NUMBER_9)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_89_fitness_0_225_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((36 - c.pct_change(2)) ** 2).rolling(window=9, min_periods=1).max()) - (((36 - c.pct_change(2)) ** 2).rolling(window=9, min_periods=1).min())))  # 原始GP表达式: TS_RANGE(SQUARED_DIFF(NUMBER_36, PRICE_CHANGE_2D), NUMBER_9)
