...
large_scale_factor_36_fitness_0_234_20250812_175508: 基于遗传规划算法生成的因子36，适应度: 0.234005
适应度分数: 0.234005
原始GP表达式: TS_EMA(TS_RANGE(LOW_CLOSE_RATIO, NUMBER_59), NUMBER_40)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_36_fitness_0_234_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((l / c.replace(0, np.nan)).rolling(window=59, min_periods=1).max()) - ((l / c.replace(0, np.nan)).rolling(window=59, min_periods=1).min()))).ewm(span=40, adjust=False).mean())  # 原始GP表达式: TS_EMA(TS_RANGE(LOW_CLOSE_RATIO, NUMBER_59), NUMBER_40)
