...
large_scale_factor_77_fitness_0_416_20250812_175508: 基于遗传规划算法生成的因子77，适应度: 0.416232
适应度分数: 0.416232
原始GP表达式: BETA5(DELTA(VOLUME_WEIGHTED_PRICE, NUMBER_27), DELTA(VOLUME_WEIGHTED_PRICE, NUMBER_23))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_77_fitness_0_416_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((amt / v.replace(0, np.nan)).diff(27)).rolling(window=5).cov(((amt / v.replace(0, np.nan)).diff(23))) / ((amt / v.replace(0, np.nan)).diff(23)).rolling(window=5).var())))  # 原始GP表达式: BETA5(DELTA(VOLUME_WEIGHTED_PRICE, NUMBER_27), DELTA(VOLUME_WEIGHTED_PRICE, NUMBER_23))
