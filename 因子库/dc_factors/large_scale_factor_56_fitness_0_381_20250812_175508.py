...
large_scale_factor_56_fitness_0_381_20250812_175508: 基于遗传规划算法生成的因子56，适应度: 0.380964
适应度分数: 0.380964
原始GP表达式: BETA20(DELTA(DAILY_RANGE_RATIO, NUMBER_9), TS_RANK(HIGH_LOW_DIFF, NUMBER_54))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_56_fitness_0_381_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((((h - l) / c.replace(0, np.nan)).diff(9)).rolling(window=20).cov(((h - l).rolling(window=54).apply(lambda s: s.rank(pct=True).iloc[-1]))) / ((h - l).rolling(window=54).apply(lambda s: s.rank(pct=True).iloc[-1])).rolling(window=20).var())))  # 原始GP表达式: BETA20(DELTA(DAILY_RANGE_RATIO, NUMBER_9), TS_RANK(HIGH_LOW_DIFF, NUMBER_54))
