...
large_scale_factor_96_fitness_0_273_20250812_175508: 基于遗传规划算法生成的因子96，适应度: 0.272657
适应度分数: 0.272657
原始GP表达式: TS_SUM(CORRELATION20(PRICE_EFFICIENCY, HIGH_CLOSE_RATIO), NUMBER_14)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_96_fitness_0_273_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((c - o) / (h - l).replace(0, np.nan)).rolling(window=20).corr((h / c.replace(0, np.nan)))).rolling(window=14, min_periods=1).sum())  # 原始GP表达式: TS_SUM(CORRELATION20(PRICE_EFFICIENCY, HIGH_CLOSE_RATIO), NUMBER_14)
