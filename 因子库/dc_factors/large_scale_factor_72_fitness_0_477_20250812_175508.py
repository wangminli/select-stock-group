...
large_scale_factor_72_fitness_0_477_20250812_175508: 基于遗传规划算法生成的因子72，适应度: 0.477125
适应度分数: 0.477125
原始GP表达式: TS_EMA(MINIMUM(VOLUME_PRICE_RATIO, TURNOVER_RATIO), NUMBER_7)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_72_fitness_0_477_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.minimum(v / np.abs(c - o).replace(0, np.nan), v / mc_circ.replace(0, np.nan))).ewm(span=7, adjust=False).mean())  # 原始GP表达式: TS_EMA(MINIMUM(VOLUME_PRICE_RATIO, TURNOVER_RATIO), NUMBER_7)
