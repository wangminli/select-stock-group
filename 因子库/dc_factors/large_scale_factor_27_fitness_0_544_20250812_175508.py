...
large_scale_factor_27_fitness_0_544_20250812_175508: 基于遗传规划算法生成的因子27，适应度: 0.544243
适应度分数: 0.544243
原始GP表达式: MULTIPLY(MAXIMUM(AMOUNT_TO_MCAP, TURNOVER_RATIO), NUMBER_35)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_27_fitness_0_544_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.maximum(amt / mc_total.replace(0, np.nan), v / mc_circ.replace(0, np.nan)) * 35))  # 原始GP表达式: MULTIPLY(MAXIMUM(AMOUNT_TO_MCAP, TURNOVER_RATIO), NUMBER_35)
