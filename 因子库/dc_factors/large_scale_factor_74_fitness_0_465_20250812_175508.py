...
large_scale_factor_74_fitness_0_465_20250812_175508: 基于遗传规划算法生成的因子74，适应度: 0.465060
适应度分数: 0.465060
原始GP表达式: MULTIPLY(TS_RANGE(STOCH_K_BASE, NUMBER_48), SPEARMAN10(HLCC_WEIGHTED, VOLUME))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_74_fitness_0_465_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((((c - l) / (h - l).replace(0, np.nan)).rolling(window=48, min_periods=1).max()) - (((c - l) / (h - l).replace(0, np.nan)).rolling(window=48, min_periods=1).min())) * ((h + l + 2*c) / 4).rolling(window=10).corr((v), method="spearman")))  # 原始GP表达式: MULTIPLY(TS_RANGE(STOCH_K_BASE, NUMBER_48), SPEARMAN10(HLCC_WEIGHTED, VOLUME))
