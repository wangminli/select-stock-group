...
large_scale_factor_33_fitness_0_273_20250812_175508: 基于遗传规划算法生成的因子33，适应度: 0.272550
适应度分数: 0.272550
原始GP表达式: TS_STD(MINIMUM(HIGH_CLOSE_RATIO, AMOUNT_TO_MCAP), NUMBER_33)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_33_fitness_0_273_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.minimum(h / c.replace(0, np.nan), amt / mc_total.replace(0, np.nan))).rolling(window=33, min_periods=1).std())  # 原始GP表达式: TS_STD(MINIMUM(HIGH_CLOSE_RATIO, AMOUNT_TO_MCAP), NUMBER_33)
