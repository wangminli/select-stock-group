...
large_scale_factor_81_fitness_0_383_20250812_175508: 基于遗传规划算法生成的因子81，适应度: 0.382976
适应度分数: 0.382976
原始GP表达式: TS_MA(BETA10(OPEN_POSITION, LOW_OPEN_RATIO), NUMBER_11)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_81_fitness_0_383_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((((((o - l) / (h - l).replace(0, np.nan)).rolling(window=10).cov((l / o.replace(0, np.nan))) / (l / o.replace(0, np.nan)).rolling(window=10).var()))).rolling(window=11, min_periods=1).mean())  # 原始GP表达式: TS_MA(BETA10(OPEN_POSITION, LOW_OPEN_RATIO), NUMBER_11)
