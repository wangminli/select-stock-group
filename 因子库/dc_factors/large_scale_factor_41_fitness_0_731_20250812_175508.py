...
large_scale_factor_41_fitness_0_731_20250812_175508: 基于遗传规划算法生成的因子41，适应度: 0.730653
适应度分数: 0.730653
原始GP表达式: MAXIMUM(CORRELATION10(LIUTONGSHI, VOLUME), SPEARMAN20(VOLUME, TRUE_RANGE))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_41_fitness_0_731_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.maximum((mc_circ).rolling(window=10).corr((v)), (v).rolling(window=20).corr((h - l), method="spearman")))  # 原始GP表达式: MAXIMUM(CORRELATION10(LIUTONGSHI, VOLUME), SPEARMAN20(VOLUME, TRUE_RANGE))
