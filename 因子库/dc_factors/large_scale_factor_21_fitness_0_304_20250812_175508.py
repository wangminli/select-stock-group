...
large_scale_factor_21_fitness_0_304_20250812_175508: 基于遗传规划算法生成的因子21，适应度: 0.304039
适应度分数: 0.304039
原始GP表达式: MINIMUM(TS_MEDIAN(HIGH_OPEN_RATIO, NUMBER_54), ABS_DIFF(NUMBER_5, VOLUME_PRICE_RATIO))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_21_fitness_0_304_20250812_175508')
    ...
    # 计算因子值
    factor_value = (np.minimum((h / o.replace(0, np.nan)).rolling(window=54, min_periods=1).median(), np.abs(5 - v / np.abs(c - o).replace(0, np.nan))))  # 原始GP表达式: MINIMUM(TS_MEDIAN(HIGH_OPEN_RATIO, NUMBER_54), ABS_DIFF(NUMBER_5, VOLUME_PRICE_RATIO))
