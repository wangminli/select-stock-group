...
large_scale_factor_59_fitness_0_242_20250812_175508: 基于遗传规划算法生成的因子59，适应度: 0.242394
适应度分数: 0.242394
原始GP表达式: TS_MIN(CORRELATION5(VOLUME_CHANGE_1D, STOCH_K_BASE), NUMBER_45)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_59_fitness_0_242_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v.pct_change(1)).rolling(window=5).corr(((c - l) / (h - l).replace(0, np.nan)))).rolling(window=45, min_periods=1).min())  # 原始GP表达式: TS_MIN(CORRELATION5(VOLUME_CHANGE_1D, STOCH_K_BASE), NUMBER_45)
