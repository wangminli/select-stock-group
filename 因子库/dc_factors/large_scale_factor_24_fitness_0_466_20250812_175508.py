...
large_scale_factor_24_fitness_0_466_20250812_175508: 基于遗传规划算法生成的因子24，适应度: 0.466257
适应度分数: 0.466257
原始GP表达式: TS_SUM(ADD(PRICE_CHANGE_2D, TURNOVER_RATIO), NUMBER_14)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_24_fitness_0_466_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((c.pct_change(2) + v / mc_circ.replace(0, np.nan))).rolling(window=14, min_periods=1).sum())  # 原始GP表达式: TS_SUM(ADD(PRICE_CHANGE_2D, TURNOVER_RATIO), NUMBER_14)
