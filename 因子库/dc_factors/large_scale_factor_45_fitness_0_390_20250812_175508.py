...
large_scale_factor_45_fitness_0_390_20250812_175508: 基于遗传规划算法生成的因子45，适应度: 0.390375
适应度分数: 0.390375
原始GP表达式: DIVIDE(MULTIPLY(AVG_PRICE, LOW_CLOSE_DEVIATION), TS_MIN(HLC_TYPICAL, NUMBER_13))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_45_fitness_0_390_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan) * (c - l) / c.replace(0, np.nan)) / ((h + l + c) / 3).rolling(window=13, min_periods=1).min()))  # 原始GP表达式: DIVIDE(MULTIPLY(AVG_PRICE, LOW_CLOSE_DEVIATION), TS_MIN(HLC_TYPICAL, NUMBER_13))
