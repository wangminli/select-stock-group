...
large_scale_factor_67_fitness_0_605_20250812_175508: 基于遗传规划算法生成的因子67，适应度: 0.605301
适应度分数: 0.605301
原始GP表达式: CORRELATION5(MAXIMUM(HLCC_WEIGHTED, OC_MID), SUBTRACT(NUMBER_7, AMOUNT))
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_67_fitness_0_605_20250812_175508')
    ...
    # 计算因子值
    factor_value = ((np.maximum((h + l + 2*c) / 4, (o + c) / 2)).rolling(window=5).corr(((7 - amt))))  # 原始GP表达式: CORRELATION5(MAXIMUM(HLCC_WEIGHTED, OC_MID), SUBTRACT(NUMBER_7, AMOUNT))
