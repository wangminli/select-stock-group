...
large_scale_factor_8_fitness_0_284_20250812_175508: 基于遗传规划算法生成的因子8，适应度: 0.284145
适应度分数: 0.284145
原始GP表达式: TS_RANK(TS_RANGE(VOLUME, NUMBER_8), NUMBER_38)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_8_fitness_0_284_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((((v).rolling(window=8, min_periods=1).max()) - ((v).rolling(window=8, min_periods=1).min()))).rolling(window=38).apply(lambda s: s.rank(pct=True).iloc[-1]))  # 原始GP表达式: TS_RANK(TS_RANGE(VOLUME, NUMBER_8), NUMBER_38)
