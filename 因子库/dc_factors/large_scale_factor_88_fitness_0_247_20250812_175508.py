...
large_scale_factor_88_fitness_0_247_20250812_175508: 基于遗传规划算法生成的因子88，适应度: 0.246864
适应度分数: 0.246864
原始GP表达式: RETURNS(TS_STD(DIRECTIONAL_VOLUME, NUMBER_43), NUMBER_57)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_88_fitness_0_247_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((v * np.sign(c - o)).rolling(window=43, min_periods=1).std()).pct_change(57))  # 原始GP表达式: RETURNS(TS_STD(DIRECTIONAL_VOLUME, NUMBER_43), NUMBER_57)
