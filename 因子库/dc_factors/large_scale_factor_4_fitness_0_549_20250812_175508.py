...
large_scale_factor_4_fitness_0_549_20250812_175508: 基于遗传规划算法生成的因子4，适应度: 0.548740
适应度分数: 0.548740
原始GP表达式: SQUARED_DIFF(CORRELATION5(VWAP_PROXY, OC_MID), NUMBER_59)
...
def add_factor(df: pd.DataFrame, param=None, **kwargs) -> (pd.DataFrame, dict):
    ...
    """
    col_name = kwargs.get('col_name', 'large_scale_factor_4_fitness_0_549_20250812_175508')
    ...
    # 计算因子值
    factor_value = (((amt / v.replace(0, np.nan)).rolling(window=5).corr(((o + c) / 2)) - 59) ** 2)  # 原始GP表达式: SQUARED_DIFF(CORRELATION5(VWAP_PROXY, OC_MID), NUMBER_59)
